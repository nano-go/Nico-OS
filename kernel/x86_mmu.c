...
#define GDT_ATTR_SYS_CODE                                                                          \
    (GDT_G_4KB | GDT_D_32 | GDT_L_32 | GDT_P_1 | GDT_DPL_KERNEL | GDT_S_NORMAL | GDT_TYPE_CODE)

// System Data Segment.
#define GDT_ATTR_SYS_DATA                                                                          \
    (GDT_G_4KB | GDT_D_32 | GDT_L_32 | GDT_P_1 | GDT_DPL_KERNEL | GDT_S_NORMAL | GDT_TYPE_DATA)

// User Code Segment.
#define GDT_ATTR_USER_CODE                                                                         \
    (GDT_G_4KB | GDT_D_32 | GDT_L_32 | GDT_P_1 | GDT_DPL_USER | GDT_S_NORMAL | GDT_TYPE_CODE)

// User Data Segment.
#define GDT_ATTR_USER_DATA                                                                         \
    (GDT_G_4KB | GDT_D_32 | GDT_L_32 | GDT_P_1 | GDT_DPL_USER | GDT_S_NORMAL | GDT_TYPE_DATA)

// Byte Unit Data Segement.
#define GDT_ATTR_BYTE_DATA                                                                         \
    (GDT_G_BYTE | GDT_D_32 | GDT_L_32 | GDT_P_1 | GDT_DPL0 | GDT_S_NORMAL | GDT_TYPE_DATA)

#define GDT_ATTR_TSS (GDT_P_1 | GDT_DPL0 | GDT_S_SYS | GDT_TYPE_TSS)


#define GDT_DESC_NULL                                                                              \
    { .limit_low = 0, .base_low = 0, .desc_high = 0 }
#define MAKE_GDT_DESC(base_addr, limit, attrs)                                                     \
    {                                                                                              \
        .limit_low = (uint16_t)(limit), .base_low = (uint16_t)(base_addr),                         \
        .desc_high = ((attrs) | (((base_addr) >> 16) & 0xFF) | ((limit) &0x000F0000) |             \
...
void mmu_init() {
    struct gdt_desc tss_desc = MAKE_GDT_DESC((uint32_t) &tss, sizeof(tss), GDT_ATTR_TSS);
