...
int proc_fork() {
	...
	child =
		kthread_create(proc_entry, NULL, parent->priority, "%s", parent->name);
	if (child == NULL) {
		return -1;
	}
...
static bool init_user_stack(struct task_struct *proc, void *user_entry) {
	...
	// Clear user stack.
	if (!vm_setrange(proc->vm, (void *) BOTTOM_USER_STACK, 0,
...
static struct task_struct *create_initproc(void *entry_addr) {
	struct task_struct *proc =
		kthread_create(proc_entry, NULL, PROC_PRIORITY_DEFAULT, "init");
	if (proc == NULL) {
		goto bad;
	}
