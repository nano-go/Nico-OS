...
/**
 * Causes the current task to yield execution to another task that is ready to
 * run on the CPU.
 */
void task_yield();

/**
 * Block the current task until it is woken up by a task. This will change the
 * task's state to TASK_BLOCKED.
 */
void task_block();
 ...
 *
 * @param status - the exited task's status-code will be filled to the "status"
 * pointer if it is not NULL.
 *
 * @return the exited task's pid or -1 if there are no child tasks for the
 * current task.
 */
