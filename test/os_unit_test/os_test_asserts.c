...
void _assert_int_equal(COMMON_PARAMETERS, int expected, int actual) {
	if (expected != actual) {
		failed(PASS_COMMON_PARAMETERS, "Expected the int value %d, but was %d",
			   expected, actual);
	}
}
...
void _assert_int_not_equal(COMMON_PARAMETERS, int unexpected, int actual) {
	if (unexpected == actual) {
		failed(PASS_COMMON_PARAMETERS, "Unexpected the int value %d",
			   unexpected);
	}
}
...
void _assert_ptr_equal(COMMON_PARAMETERS, void *expected, void *actual) {
	if (expected != actual) {
		failed(PASS_COMMON_PARAMETERS, "Expected the ptr 0x%x, but was 0x%x",
...
void _assert_ptr_not_equal(COMMON_PARAMETERS, void *unexpected, void *actual) {
	if (unexpected == actual) {
		failed(PASS_COMMON_PARAMETERS, "Unexpected the ptr 0x%x",
			   (uint32_t) unexpected);
	}
}
...
void _assert_str_equal(COMMON_PARAMETERS, char *expected, char *actual) {
	if (strcmp(expected, actual) != 0) {
		failed(PASS_COMMON_PARAMETERS,
			   "Expected the string \"%s\", but was \"%s\"", expected, actual);
	}
}
...
void _assert_str_not_equal(COMMON_PARAMETERS, char *unexpected, char *actual) {
	if (strcmp(unexpected, actual) == 0) {
		failed(PASS_COMMON_PARAMETERS,
			   "Unexpected the string \"%s\"", unexpected);
	}
}


void _assert_true(COMMON_PARAMETERS, bool condition,
				   const char *condition_str) {
	if (!condition) {
		failed(PASS_COMMON_PARAMETERS, "The <%s> condition should be true",
			   condition_str);
	}
}
...
void _assert_false(COMMON_PARAMETERS, bool condition,
					const char *condition_str) {
	if (condition) {
		failed(PASS_COMMON_PARAMETERS, "The <%s> condition should be false",
			   condition_str);
	}
}
