...
static void skipelem_test() {
    ...
    struct skipelem_tc {
        ...
        int nelement;            // Number of elements.
                     ...
                     {"abc/./b", {"abc", ".", "b"}, {3, 5, 7}, 3},
                     {"//abc//\n../b/", {"abc", "\n..", "b"}, {5, 10, 12}, 3},
                     {"///abc//../  d//efg../", {"abc", "..", "  d", "efg.."}, {6, 10, 14, 21}, 4}};
...
static void path_lookup_test() {
    ...
    struct path_lookup_tc {
        ...
        {"tmp/foo/bar", {"/tmp/foo/bar", "/tmp/../tmp/foo/bar", 0}, 2, false, 0},
        {"tmp/foo/bar",
         {"/tmp/foo/bar/foo", "/tmp/.././/../foo/./bar//", "/tmp/./foo/bar/../foo",
          "/tmp/....../foo/bar", 0},
         -1,
         false,
         0},
    ...
    for (uint i = 0; i < sizeof testcases / sizeof *testcases; i++) {
        ...
        while (*input != 0) {
            ...
            if (tc->found_elemidx == -1) {
                ...
                assert_int_equal(r.path_elements[tc->found_elemidx]->inum, ip->inum);
            }
            input++;
            log_end_op(log);
        }
